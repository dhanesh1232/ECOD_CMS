        ...
        {mobileMenuOpen && (
          <>
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 bg-black/60 backdrop-blur-sm z-20 flex justify-end items-center"
              onClick={() => setMobileMenuOpen(false)}
              role="presentation"
            >
              <div className="w-1/4 h-full flex items-center justify-center cursor-pointer">
                <span className="dark:text-white text-gray-100 -rotate-90">
                  Click here
                </span>
              </div>
            </motion.div>
  ...
  const SidebarContent = () => (
    <LayoutGroup>
      <motion.nav
        ...
        className={`flex flex-col h-full backdrop-blur-lg bg-indigo-800/95 dark:bg-gray-900/95 text-white shadow-xl border-r border-indigo-700/50 dark:border-gray-800 ${
          isMobile ? "fixed z-30 w-64" : "relative"
        } ${!isMobile && collapsed && "z-[100]"}`}
        ref={navRef}
      ...
      >
        ...
        <div className="p-4 py-4 flex items-center justify-between border-b border-indigo-700/50 relative">
          ...
          >
            ...
            >
              <ChatBotAI size={isMobile ? 20 : 24} />
            </motion.div>
            {(!collapsed || isMobile) && (
              <span
                className={`${
                  isMobile ? "text-lg" : "text-xl"
                } ml-1 font-bold text-white tracking-tight`}
              >
                ECODrIx
              </span>
            )}
                  ...
                  <TooltipTrigger asChild>
                    ...
                    >
                      {React.cloneElement(item.icon, {
                        primaryColor: isActive ? "#E0E7FF" : "#A5B4FC",
                        secondaryColor: isActive ? "#818CF8" : "#6366F1",
